...
transplot <- function(transmission, style = 1, vcolor = "lightblue", vsize = 12, label.cex = 1.5, showlabel = FALSE, dateLastSample = 2008) {
  ...
  if(style==1){
    ...
    # convert NA infectors (external) to "source"
    edges_plot$infector[is.na(edges_plot$infector)] <- "source"
    edges_plot$infector[which(edges_plot$prob<0.1)] <- "source"
  }
  ...
  if(style==1 | style==2){
    ...
    root_name <- V(g)$name[is.na(match(V(g)$name,as.character(edges_plot$infectee)))][1]
  ...
  if (style == 1) { # network style plot
    ...
    layout <- layout_as_tree(g, root = which(V(g)$name == "source"))
    plot(
      g,
      layout = layout,
      #edge.lty = edge_type,
